		...
		while (*pszMsg && pszOut < pszEnd) {
			if (*pszMsg == '%') {
				...
				else if (*pszMsg == 'p')
				{
					...
					if ((INT64)value == (INT64)-1 || (INT64)value == (INT64)-2)
					{
						if (*pszMsg == 'p')
							pszMsg++;
...
static BOOL syelogIsOpen(PFILETIME pftLog)
{
	...
	s_hPipe = CreateFileW(SYELOG_PIPE_NAMEW, GENERIC_WRITE, 0, NULL, OPEN_EXISTING, SECURITY_ANONYMOUS, NULL);
	if (s_hPipe != INVALID_HANDLE_VALUE)
	{
	...
	// Wait 2 seconds.
	if (WaitNamedPipeW(SYELOG_PIPE_NAMEW, 2000))
	{
...
VOID SyelogKey(std::vector<BYTE> key, BOOL useOldDxArc)
{
	...
	// Initilaize the message to all zeros
	...
	// memcpy the key into the message
	memcpy(Message.szMessage, key.data(), key.size() * sizeof(BYTE));

	const PCHAR pszEnd = Message.szMessage + key.size() + 1;

	Message.nBytes = (USHORT)(pszEnd - ((PCSTR)&Message));

	Message.fUseOldDxArc = useOldDxArc;
	Message.fIsKey = TRUE;
...
VOID SyelogExV(BOOL fTerminate, BYTE nSeverity, PCSTR pszMsgf, va_list args)
{
	...
	*pszEnd = '\0';
	VSafePrintf(pszMsgf, args, pszBuf, (int)(Message.szMessage + sizeof(Message.szMessage) - 1 - pszBuf));
